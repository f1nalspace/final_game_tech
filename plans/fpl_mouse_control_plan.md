...
## 0. Handover (Stand 2026-09-25, nach Iteration 4)
...
- **Als Nächstes: Iteration 5 (was qemu darüber hinaus braucht: `scanCode`, Maustasten 8/9, horizontales Rad, Enter/Leave),** die Schrittliste steht in 0.6.
...
### 0.6 Nächster Schritt: Iteration 5 (was qemu darüber hinaus braucht) als Schrittliste

1. **API:** `fplKeyboardEvent.scanCode` (`uint32_t`, PC-Satz 1, erweiterte Tasten als `0xE0xx`, Pause als `0xE11D`, 0 = unbekannt, 7.1). `fplMouseEvent.wheelDeltaX` für das horizontale Rad. `fplMouseEventType_Enter`/`fplMouseEventType_Leave`. `TestSizes` in `FPL_Test`: `fplEvent` darf nicht wachsen (8).
2. **Win32 `scanCode`:** aus `lParam` Bits 16–23, Bit 24 (erweitert) ergibt das Präfix `0xE0`, Pause und Druck als Sonderfälle. Der Hook-Weg liefert denselben Wert aus `KBDLLHOOKSTRUCT.scanCode` und `LLKHF_EXTENDED`. Ein Scancode 0 (manche eingespeisten Tasten) über `MapVirtualKeyW(vk, MAPVK_VK_TO_VSC_EX)` nachschlagen.
3. **X11 `scanCode`:** evdev = X-Keycode − 8, Tabelle evdev → Satz 1. 1–83 sind gleich, der Rest (etwa 40 Einträge: Ziffernblock-Enter 96 → `0xE01C`, RStrg 97 → `0xE01D`, Ziffernblock-/ 98 → `0xE035`, SysRq 99 → `0xE037`, RAlt 100 → `0xE038`, Pos1/Pfeile/Bild/Einfg/Entf 102–111 → `0xE047`…`0xE053`, `<>` 86 → `0x56`, F11/F12 87/88 → `0x57`/`0x58`, Pause 119 → `0xE11D`, LMeta/RMeta/Menü 125–127 → `0xE05B`…`0xE05D`) als benannte Tabelle ohne Doppelungen.
4. **Maus:** X11-Tasten 8/9 → `X1`/`X2`, Tasten 6/7 und Win32 `WM_MOUSEHWHEEL` → `wheelDeltaX`. Dabei den Befund `WM_MOUSEWHEEL`-Position (1.3) gleich mit beheben (`ScreenToClient`).
5. **Enter/Leave:** X11 `EnterNotify`/`LeaveNotify` nur mit `mode == NotifyNormal` und ohne `detail == NotifyInferior` (eigene Grabs erzeugen `NotifyGrab`/`NotifyUngrab`). Win32 `WM_MOUSELEAVE` (`TrackMouseEvent` gibt es schon) und das erste `WM_MOUSEMOVE` danach. Im relativen Modus und beim Maus-Grab kommen keine.
6. **Demo:** `scan=0x…` in der Tastenzeile, `wheel dx=`/`dy=`, `mouse enter`/`mouse leave`.
7. **Tests** (vorher fragen): `scan_codes_layout_independent` schickt über python-xlib XTEST Keycodes (nicht Keysyms) einer 105er-Tastatur und prüft sie gegen eine Erwartungstabelle, einmal mit `setxkbmap us` und einmal mit `setxkbmap de`. **Das ändert das Layout des Nutzers kurz**, danach zurück auf `de` (vorher mit `setxkbmap -query` sichern). `mouse_side_buttons` (`xdotool click 8`/`9`), `mouse_horizontal_wheel` (`click 6`/`7`), `mouse_enter_leave` (einmal pro Übergang, nicht beim eigenen Grab). Win32-Scancodes: Handprüfung mit der Demo, unter wine als Vortest.
8. **Doku und Changelog**, Build-Matrix, Tests (mit Freigabe), „Stand“ unter Iteration 5, dann fragen, ob committet werden soll.
