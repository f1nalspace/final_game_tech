...
## 0. Handover (Stand 2026-09-25, nach Iteration 5)
...
- **Als Nächstes: Iteration 6 (Abschluss: Doku-Seite, Demo-Readme mit Handprüfliste, `FPL_Test`-Prüfungen, gesammelte Windows-Abnahme),** die Schrittliste steht in 0.6.
...
1. **Doku:** eine Seite oder ein Abschnitt mit der qemu-artigen Schleife aus 2.1 (Tastatur-Grab + relativer Modus auf Strg+Alt+G, Deltas an den Gast, `scanCode` an den Gast, Enter/Leave), Querverweise zwischen den Abschnitten `…_cursor_warp`, `…_cursor_grab`, `…_cursor_relative`, `…_keyboard_grab` und den Event-Abschnitten. Changelog v1.0.1 auf Vollständigkeit prüfen.
2. **Demo-Readme** `demos/FPL_InputGrab/README.md` mit Tastenkürzeln, Parametern, Log-Format und der **Handprüfliste für Windows** aus 4.4 (samt Iteration 5).
3. **`FPL_Test` ohne Fenster:** `fpl__X11GetScanCode` (Tabelle ohne Doppelungen, sortiert, keine Satz-1-Codes im Bereich 1–83, Stichproben wie im Ad-hoc-Check dieser Iteration), `fpl__Win32GetScanCode` (die 16 Fälle des Ad-hoc-Checks, nur unter Win32), `sizeof(fplKeyboardEvent)`, `sizeof(fplMouseEvent)` und `sizeof(fplEvent)` in `TestSizes` (heute 32/32/120 auf x64), die Umrechnung absolut → relativ und die Restsammlung. `FPL_Test` ist auf `develop` rot (1.3), die neuen Prüfungen müssen für sich grün sein.
4. **Gesammelte Windows-Abnahme** (7.1) mit Handprüfliste 4.4 und Selbsttest, dazu der qemu-Gast mit usb-tablet. Danach Befunde in 1.3.
5. Build-Matrix, alle Tests (vorher fragen), „Stand“ unter Iteration 6, dann fragen, ob committet werden soll. Offen bleibt der eigene Branch `fpl/win32-init-events` (7.1), der sich vor oder nach Iteration 6 einschieben lässt.
