...
- `--selftest` läuft mit **1265 Prüfungen** sauber unter AddressSanitizer und UndefinedBehaviorSanitizer durch, davon ein kopfloser Rahmen, der Tasten mit allen drei Modifiern drückt, tippt, mit der mittleren und mit der linken Maustaste klickt und die Antwort zurückliest.
...
**Abschaltbar, standardmäßig aus** — `toggles.wordWrap`, umgesetzt in Iteration 7. Ist er aus, ist eine Dokumentzeile genau eine Bildschirmzeile, Zeile N ist in O(1) erreichbar, und der zweite Index wird gar nicht erst gebaut.
...
- Optionaler Zeilenumbruch: zweiter Index Bildschirmzeile ↔ Dokumentzeile. Zeilennummer nur an der ersten Bildschirmzeile einer Dokumentzeile. ✅
...
**Abnahme des Umbruchs:** *Erfüllt.* `[the caret follows the rows]` schaltet den Umbruch mit gesetztem Cursor **und** gesetzter Auswahl an und wieder aus und hält beide Offsets dagegen — sie sind Offsets ins Dokument, und der Umbruch ändert nur die Ansicht. Dazu `[breaking lines to fit]`, das die Zeilen gegen den Text hält: jede Bildschirmzeile fängt dort an, wo die davor aufgehört hat, die letzte endet, wo die Dokumentzeile endet, keine ist breiter als die Umbruchbreite, und eine Zeile mit Leerzeichen wird **an einem Leerzeichen** getrennt — die letzte Zusage ist die, die ein bloßes Hartabschneiden von einem Wortumbruch unterscheidet. `[the wrap index across an edit]` fährt vier verschiedene Änderungen über ein Dokument von 800 Zeilen und vergleicht den mitgeführten Index Eintrag für Eintrag mit einem, der aus demselben Text **neu** gerechnet wurde. Und `[breaking a real file]` bricht `final_ui.h` selbst um: 14 474 Zeilen werden zu 26 556 Bildschirmzeilen.

**Gemessen:** Einmal alles umbrechen kostet **1,8 ms**; der Frame nach *einem* Tastendruck kostet **0,012 ms**, weil nur die geänderte Zeile neu gemessen wird.
...
**Was beim Umbruch anders lief als geplant:**

- **Der zweite Index wird nicht nur bei Breitenänderung neu gebaut, sondern bei jeder Änderung *nachgeführt*.** Der Plan sagt „bei Breitenänderung neu gebaut", und das allein hieße: jeder Tastendruck kostet einen Messlauf über das ganze Dokument. Gemessen wären das 1,8 ms — je Tastendruck, auf 14 000 Zeilen. Also merkt sich eine Änderung nur, *welche* Zeilen sie berührt hat (messen braucht eine Schrift, und die gehört dem Kontext, den `fuiEditorInsert` nicht hat), und der nächste Build misst genau die nach. 0,012 ms.
- **Zeilenzahlen je Zeile, Summen je Block — keine laufende Summe.** Eine laufende Summe fände eine Bildschirmzeile in O(1), müsste aber bei jeder Änderung von der geänderten Zeile bis zum Dokumentende fortgeschrieben werden. Blocksummen zu 256 Zeilen kosten eine Addition je Block und finden eine Bildschirmzeile per Binärsuche plus einem Blockdurchlauf.
- **Alles, was zeichnet und misst, bekommt einfach einen *engeren Bereich*.** Das war die Überraschung: `fuiEditor__DrawLine`, `DistanceOfOffset` und `OffsetAtDistance` nehmen seit Iteration 1 ein Paar `(lineStart, lineEnd)`. Eine Bildschirmzeile ist ein Stück einer Dokumentzeile — also reicht es, ihnen dieses Stück zu geben, und Auswahl, Suchtreffer, Bereichs-Dekorationen und der Cursor stimmen von selbst. Nur die Einfärbung nicht: ein Lexer färbt *Zeilen*, seine Styles sind also weiter von der Zeile aus indiziert. Dafür trägt `fuiEditor__LinePaint` jetzt ein `styleBaseOffset`.
- **Ein Cursor auf einer Umbruchstelle gehört zwei Zeilen an.** Derselbe Offset ist das Ende der einen und der Anfang der nächsten — ein Offset, zwei Stellen auf dem Bildschirm. Ohne eine Entscheidung darüber sprang die Ende-Taste sichtbar an den *Anfang der nächsten* Zeile. Es gibt jetzt `caretIsAtARowEnd`: jede Bewegung setzt es zurück, nur die Ende-Taste setzt es. Gefunden von der Prüfung, nicht beim Lesen.
- **Die Randspalte wird nach den *Dokument*zeilen bemessen, und der Balkenstreifen ist beim Umbruch immer reserviert.** Beides ist derselbe Kreis: wie viele Bildschirmzeilen es gibt, hängt davon ab, wie breit der Text sein darf — und das hängt daran, was Randspalte und Balken übriglassen. Beim senkrechten Balken flackert der Kreis obendrein genau dann, wenn das Dokument so hoch ist wie der Kasten.
- **Zwei Prüfungen waren blind.** Ein Dokument aus fünf Zeilen bleibt im ersten Block, und die Blocksummen sind dann alle null und alle richtig, egal was der Code tut — die Prüfung läuft jetzt über 800 Zeilen. Und „die Bildschirmzeilen decken die Zeile ab und sind nicht zu breit" ist genau das, was ein Abschneiden mitten im Wort auch erfüllt — geprüft wird jetzt, dass vor jedem Umbruch ein Leerzeichen steht.
- **Und wieder wurde jede neue Absicherung absichtlich kaputtgemacht und die Suite dabei rot gesehen** — die fehlende Cursor-Affinität (1 Fehler), die nicht vermerkte Änderung (5), der nicht mitgeschobene saubere Rest (5), die nicht nachgeführten Blocksummen (4), der Umbruch nie am Leerzeichen (1), die Binärsuche über die Blöcke (1), Hoch/Runter wieder in Dokumentzeilen (2) und die Nummer auf jeder Bildschirmzeile (1).
