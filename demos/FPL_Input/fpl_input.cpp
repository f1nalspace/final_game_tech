		...
		if (fglLoadOpenGL(true)) {
			...
			while (fplWindowUpdate()) {
					...
					switch (ev.type) {
						case fplEventType_Mouse:
						{
							if (ev.mouse.type == fplMouseEventType_Move) {
								...
								input.lastMouseWheelUpdateTime = fplTimeMilliseconds();
							}
						} break;
				...
				if (input.lastMouseWheelUpdateTime > 0) {
					if (fplTimeMilliseconds() - input.lastMouseWheelUpdateTime >= maxWheelShowTime) {
				...
				if (input.lastTextCursorBlinkTime == 0) {
					input.lastTextCursorBlinkTime = fplTimeMilliseconds();
					input.showTextCursor = true;
				} else {
					if ((fplTimeMilliseconds() - input.lastTextCursorBlinkTime) >= maxCursorShowTime) {
