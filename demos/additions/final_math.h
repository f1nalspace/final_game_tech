...
static const float F32MinValue = 1.175494351e-38f;
//! Ratio of a circle's circumference to its diameter (Pi).
static const float F32Pi = 3.14159265358979323846f;
//! Full turn in radians (2*Pi).
static const float F32Tau = 6.28318530717958647692f;
//! Factor to convert degrees to radians.
static const float F32Deg2Rad = 3.14159265358979323846f / 180.0f;
//! Factor to convert radians to degrees.
static const float F32Rad2Deg = 180.0f / 3.14159265358979323846f;
//! Smallest difference between two representable floats (FLT_EPSILON).
static const float F32Epsilon = 1.192092896e-07f;
//! Reciprocal of 255, to map a byte 0..255 into 0..1.
static const float F32InvByte = 1.0f / 255.0f;
...
*/
fpl_extern_inline Ratio64 Ratio64Init(double numerator, double denominator) {
...
*/
fpl_extern_inline double Ratio64Compute(const Ratio64 ratio) {
...
*/
fpl_extern_inline Vec2i V2iZero() {
...
*/
fpl_extern_inline Vec2i V2iCopy(const Vec2i v) {
...
*/
fpl_extern_inline Vec2i V2iInit(const int x, const int y) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec2i V2iInitScalar(const int value) {
...
*/
fpl_extern_inline Vec2u V2uZero() {
...
* @return A copy of the input vector.
*/
fpl_extern_inline Vec2u V2uCopy(const Vec2u v) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec2u V2uInit(const uint32_t x, const uint32_t y) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec2u V2uInitScalar(const uint32_t value) {
...
fpl_extern_inline Vec2u V2u() {
	return V2uZero();
}
//! C++ overload constructing a Vec2u from a single scalar.
fpl_extern_inline Vec2u V2u(const uint32_t value) {
	return V2uInitScalar(value);
}
//! C++ overload constructing a Vec2u from two components.
fpl_extern_inline Vec2u V2u(const uint32_t x, const uint32_t y) {
	return V2uInit(x, y);
}
#endif
...
*/
fpl_extern_inline Vec2f V2fZero() {
...
* @return A copy of the input vector.
*/
fpl_extern_inline Vec2f V2fCopy(const Vec2f v) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec2f V2fInit(const float x, const float y) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec2f V2fInitScalar(const float value) {
...
*/
fpl_extern_inline Vec2f V2fInitV2i(const Vec2i v) {
...
* @return The converted float vector.
*/
fpl_extern_inline Vec2f V2fInitV2u(const Vec2u v) {
	Vec2f result = fplStructInit(Vec2f, (float)v.x, (float)v.y);
	return(result);
}
...
*/
fpl_extern_inline Rect2f R2fInit(const Vec2f pos, const Vec2f size) {
...
*/
fpl_extern_inline Vec3f V3fZero() {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec3f V3fInitScalar(const float scalar) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec3f V3fInitXY(const Vec2f other, const float z) {
...
* @return A copy of the input vector.
*/
fpl_extern_inline Vec3f V3fCopy(const Vec3f other) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec3f V3fInit(const float x, const float y, const float z) {
...
*/
fpl_extern_inline Vec4f V4fZero() {
...
* @return A copy of the input vector.
*/
fpl_extern_inline Vec4f V4fCopy(const Vec4f other) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec4f V4fInit(const float x, const float y, const float z, const float w) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec4f V4fInitXYZ(const Vec3f v, const float w) {
...
* @return The initialized vector.
*/
fpl_extern_inline Vec4f V4fInitXY(const Vec2f v, const float z, const float w) {
...
*/
fpl_extern_inline Mat2f M2fDefault() {
...
*/
fpl_extern_inline Mat2f M2fCopy(const Mat2f other) {
...
fpl_extern_inline Mat2f M2f() {
	return M2fDefault();
}
//! C++ overload constructing a Mat2f from another Mat2f.
fpl_extern_inline Mat2f M2f(const Mat2f &other) {
...
*/
fpl_extern_inline Mat4f M4fInit(const float value) {
	Mat4f result = {
...
* @return The identity matrix.
*/
fpl_extern_inline Mat4f M4fIdentity() {
...
* @return A copy of the input matrix.
*/
fpl_extern_inline Mat4f M4fCopy(const Mat4f other) {
...
fpl_extern_inline Mat4f M4f(const float value = 1.0f) {
	return M4fInit(value);
}
//! C++ overload constructing a Mat4f from another Mat4f.
fpl_extern_inline Mat4f M4f(const Mat4f &other) {
...
*/
fpl_extern_inline Quaternion QuatInit(const float w, const float x, const float y, const float z) {
	Quaternion result;
	result.w = w;
	result.x = x;
	result.y = y;
...
*/
fpl_extern_inline Quaternion QuatIdentity() {
...
* @return The initialized quaternion.
*/
fpl_extern_inline Quaternion QuatInitSXYZ(const float s, const Vec3f axis) {
	Quaternion result;
	result.s = s;
	result.n = axis;
	return(result);
...
fpl_extern_inline Quaternion Quat() {
	return QuatIdentity();
}
//! C++ overload constructing a quaternion from its four components.
fpl_extern_inline Quaternion Quat(const float w, const float x, const float y, const float z) {
	return QuatInit(w, x, y, z);
}
//! C++ overload constructing a quaternion from a scalar and a vector part.
fpl_extern_inline Quaternion Quat(const float s, const Vec3f &axis) {
...
*/
fpl_extern_inline Viewport4i VP4iInit(const int32_t x, const int32_t y, const int32_t w, const int32_t h) {
...
* @return The initialized viewport.
*/
fpl_extern_inline Viewport4f VP4fInit(const float x, const float y, const float w, const float h) {
...
*/
fpl_extern_inline float F32NaN() {
...
*/
fpl_extern_inline float F32Infinity() {
	union { uint32_t u; float f; } bits;
	bits.u = 0x7F800000u;
	return bits.f;
}
...
*/
fpl_extern_inline bool F32IsNaN(const float x) {
...
*/
fpl_extern_inline float F32Sign(const float x) {
	if(F32IsNaN(x)) {
		return F32NaN();
	}
	if(x == 0.0) {
...
*/
fpl_extern_inline float F32Cos(const float angle) {
...
*/
fpl_extern_inline float F32Sin(const float angle) {
...
*/
fpl_extern_inline float F32Tan(const float angle) {
...
*/
fpl_extern_inline float F32ArcCos(const float x) {
...
* @return The angle in radians.
*/
fpl_extern_inline float F32ArcSin(const float x) {
...
* @return The angle in radians.
*/
fpl_extern_inline float F32ArcTan(const float x) {
...
*/
fpl_extern_inline float F32ArcTan2(const float y, const float x) {
...
*/
fpl_extern_inline float F32Abs(const float value) {
...
*/
fpl_extern_inline float F32Power(const float x, const float y) {
...
*/
fpl_extern_inline float F32Modulate(const float x, const float y) {
...
*/
fpl_extern_inline float F32Floor(const float value) {
...
*/
fpl_extern_inline float F32Ceil(const float value) {
...
*/
fpl_extern_inline float F32Min(const float a, const float b) {
...
*/
fpl_extern_inline float F32Max(const float a, const float b) {
	float result = a > b ? a : b;
	return(result);
}
/**
...
*/
fpl_extern_inline float F32SquareRoot(const float value) {
...
*/
fpl_extern_inline float F32RadiansToDegrees(const float radians) {
...
* @return The angle in radians.
*/
fpl_extern_inline float F32DegreesToRadians(const float degrees) {
...
*/
fpl_extern_inline float F32Lerp(float a, float t, float b) {
...
*/
fpl_extern_inline float F32Avg(float oldValue, float t, float newValue) {
...
*/
fpl_extern_inline float F32Clamp(float value, float min, float max) {
...
*/
fpl_extern_inline float F32GetBestAngleDistance(float a0, float a1) {
	float max = F32Pi * 2;
...
*/
fpl_extern_inline float F32AngleLerp(float a, float t, float b) {
...
*/
fpl_extern_inline float F32AngleNormalize(const float angle) {
...
*/
fpl_extern_inline uint8_t RoundF32ToU8(float value) {
...
*/
fpl_extern_inline float RoundU8ToF32(uint8_t value) {
...
* @return The sum a + b.
*/
fpl_extern_inline Vec2f V2fAdd(const Vec2f a, const Vec2f b) {
...
*/
fpl_extern_inline Vec2f V2fSub(const Vec2f a, const Vec2f b) {
	Vec2f result = V2fInit(a.x - b.x, a.y - b.y);
	return(result);
}
...
*/
fpl_extern_inline Vec2f V2fMultScalar(const Vec2f v, const float s) {
...
*/
fpl_extern_inline Vec2f V2fAddMultScalar(const Vec2f a, const Vec2f b, const float s) {
...
fpl_extern_inline Vec2f operator*(const Vec2f &v, float s) {
	Vec2f result = V2fMultScalar(v, s);
	return(result);
}

//! C++ operator multiplying a scalar by a Vec2f.
fpl_extern_inline Vec2f operator*(float s, const Vec2f &v) {
	Vec2f result = V2fMultScalar(v, s);
	return(result);
}

//! C++ compound operator scaling a Vec2f in place.
fpl_extern_inline Vec2f &operator*=(Vec2f &v, float s) {
	v = V2fMultScalar(v, s);
	return(v);
}

//! C++ unary negation of a Vec2f.
fpl_extern_inline Vec2f operator-(const Vec2f &v) {
	Vec2f result = V2fInit(-v.x, -v.y);
	return(result);
}

//! C++ operator adding two Vec2f vectors.
fpl_extern_inline Vec2f operator+(const Vec2f &a, const Vec2f &b) {
	Vec2f result = V2fAdd(a, b);
	return(result);
}

//! C++ compound operator adding a Vec2f in place.
fpl_extern_inline Vec2f &operator+=(Vec2f &a, const Vec2f &b) {
	a = V2fAdd(a, b);
	return(a);
}
...
*/
fpl_extern_inline Vec2f V2fAbs(const Vec2f v) {
...
*/
fpl_extern_inline Vec2f V2fNegate(const Vec2f v) {
...
*/
fpl_extern_inline float V2fDot(const Vec2f a, const Vec2f b) {
...
*/
fpl_extern_inline float V2fLength(const Vec2f v) {
...
*/
fpl_extern_inline Vec2f V2fNormalize(const Vec2f v) {
	float l = V2fLength(v);
	if (l == 0) {
		l = 1;
	}
...
*/
fpl_extern_inline Vec2f V2fHadamard(const Vec2f a, const Vec2f b) {
	Vec2f result = V2fInit(a.x * b.x, a.y * b.y);
	return(result);
}
...
*/
fpl_extern_inline Vec2f V2fMultMat2(const Mat2f A, const Vec2f v) {
...
*/
fpl_extern_inline float V2fDistanceSquared(const Vec2f a, const Vec2f b) {
	float dx = b.x - a.x;
	float dy = b.y - a.y;
...
*/
fpl_extern_inline Vec2f V2fPerp(const Vec2f v) {
...
*/
fpl_extern_inline Vec2f V2fCrossR(const Vec2f a, float s) {
...
*/
fpl_extern_inline Vec2f V2fCrossL(float s, const Vec2f a) {
	return V2fInit(-s * a.y, s * a.x);
}
...
*/
fpl_extern_inline float V2fCrossZ(const Vec2f a, const Vec2f b) {
...
*/
fpl_extern_inline float V2fAngleFromAxis(const Vec2f axis) {
...
*/
fpl_extern_inline Vec2f V2fAxisFromAngle(const float angle) {
...
*/
fpl_extern_inline Vec2f V2fMajorAxis(const Vec2f v) {
	float x = F32Abs(v.x);
	float y = F32Abs(v.y);
	if (x > y) {
...
*/
fpl_extern_inline Vec2f V2fRandomDirection() {
...
*/
fpl_extern_inline Vec2f V2fLerp(const Vec2f a, const float t, const Vec2f b) {
	Vec2f result;
...
*/
fpl_extern_inline Vec2f V2fMin(const Vec2f a, const Vec2f b) {
...
*/
fpl_extern_inline Vec2f V2fMax(const Vec2f a, const Vec2f b) {
...
*/
fpl_extern_inline Vec2f V2fClamp(const Vec2f value, const Vec2f min, const Vec2f max) {
...
* @return The sum a + b.
*/
fpl_extern_inline Vec2i V2iAdd(const Vec2i a, const Vec2i b) {
...
* @return The difference a - b.
*/
fpl_extern_inline Vec2i V2iSub(const Vec2i a, const Vec2i b) {
	Vec2i result = V2iInit(a.x - b.x, a.y - b.y);
	return(result);
}
...
* @return The component-wise product.
*/
fpl_extern_inline Vec2i V2iMult(const Vec2i a, const Vec2i b) {
	Vec2i result = V2iInit(a.x * b.x, a.y * b.y);
	return(result);
}
...
*/
fpl_extern_inline Vec2i V2iDiv(const Vec2i a, const Vec2i b) {
	Vec2i result = V2iInit(a.x / b.x, a.y / b.y);
	return(result);
}
...
*/
fpl_extern_inline bool V2iEquals(const Vec2i a, const Vec2i b) {
...
* @return The negated vector.
*/
fpl_extern_inline Vec3f V3fNegate(const Vec3f v) {
...
* @return The vector with absolute components.
*/
fpl_extern_inline Vec3f V3fAbs(const Vec3f v) {
...
* @return The scaled vector.
*/
fpl_extern_inline Vec3f V3fMultScalar(const Vec3f v, const float s) {
...
* @return The sum a + b.
*/
fpl_extern_inline Vec3f V3fAdd(const Vec3f a, const Vec3f b) {
...
* @return The difference a - b.
*/
fpl_extern_inline Vec3f V3fSub(const Vec3f a, const Vec3f b) {
	Vec3f result = V3fInit(a.x - b.x, a.y - b.y, a.z - b.z);
	return(result);
}
...
* @return The dot product.
*/
fpl_extern_inline float V3fDot(const Vec3f a, const Vec3f b) {
...
* @return The squared distance.
*/
fpl_extern_inline float V3fDistanceSquared(const Vec3f a, const Vec3f b) {
	float dx = b.x - a.x;
	float dy = b.y - a.y;
	float dz = b.z - a.z;
...
*/
fpl_extern_inline float V3fLength2(const Vec3f v) {
...
* @return The Euclidean length.
*/
fpl_extern_inline float V3fLength(const Vec3f v) {
...
* @return The unit-length vector, or the original vector when its length is zero.
*/
fpl_extern_inline Vec3f V3fNormalize(const Vec3f v) {
	float l = V3fLength(v);
	if (l == 0) {
		l = 1;
	}
...
*/
fpl_extern_inline Vec3f V3fCross(const Vec3f a, const Vec3f b) {
...
* @return The interpolated vector.
*/
fpl_extern_inline Vec3f V3fLerp(const Vec3f a, float t, const Vec3f b) {
	Vec3f result;
	result.x = F32Lerp(a.x, t, b.x);
	result.y = F32Lerp(a.y, t, b.y);
...
* @return The component-wise product.
*/
fpl_extern_inline Vec3f V3fHadamard(const Vec3f a, const Vec3f b) {
	Vec3f result;
	result.x = a.x * b.x;
	result.y = a.y * b.y;
	result.z = a.z * b.z;
...
fpl_extern_inline Vec3f operator*(float s, const Vec3f &v) {
	Vec3f result = V3fMultScalar(v, s);
	return(result);
}

//! C++ operator multiplying a Vec3f by a scalar.
fpl_extern_inline Vec3f operator*(const Vec3f &v, float s) {
	Vec3f result = V3fMultScalar(v, s);
	return(result);
}

//! C++ compound operator scaling a Vec3f in place.
fpl_extern_inline Vec3f &operator*=(Vec3f &v, float s) {
	v = s * v;
	return(v);
}

//! C++ operator adding two Vec3f vectors.
fpl_extern_inline Vec3f operator+(const Vec3f &a, const Vec3f &b) {
	Vec3f result = V3fAdd(a, b);
	return(result);
}

//! C++ compound operator adding a Vec3f in place.
fpl_extern_inline Vec3f &operator+=(Vec3f &a, const Vec3f &b) {
	a = a + b;
	return(a);
}
...
fpl_extern_inline Vec3f operator-(const Vec3f &v) {
...
*/
fpl_extern_inline Mat2f M2fFromAngle(float angle) {
...
* @return The rotation matrix.
*/
fpl_extern_inline Mat2f M2fFromAxis(const Vec2f axis) {
	Mat2f result;
	result.col1 = axis;
...
*/
fpl_extern_inline Mat2f M2fTranspose(const Mat2f m) {
	Mat2f result;
...
*/
fpl_extern_inline Mat2f M2fMult(const Mat2f a, const Mat2f b) {
	Mat2f result;
...
* @return The angle in radians.
*/
fpl_extern_inline float M2fToAngle(const Mat2f mat) {
...
*/
fpl_extern_inline Mat2f M2fMultTranspose(const Mat2f a, const Mat2f b) {
	Mat2f result;
...
* @return The orthographic projection matrix.
*/
fpl_extern_inline Mat4f M4fOrthoRH(const float left, const float right, const float bottom, const float top, const float zNear, const float zFar) {
	Mat4f result = M4fInit(1.0f);
	result.r[0][0] = 2.0f / (right - left);
	result.r[1][1] = 2.0f / (top - bottom);
	result.r[2][2] = -2.0f / (zFar - zNear);
...
*/
fpl_extern_inline Mat4f M4fPerspectiveRH(const float fov, const float aspect, const float zNear, const float zFar) {
...
*/
fpl_extern_inline Mat4f M4fLookAtRH(const Vec3f eye, const Vec3f center, const Vec3f up) {
	// Forward/Side/Upward
...
*/
fpl_extern_inline Mat4f M4fTranslationV2(const Vec2f p) {
	Mat4f result = M4fInit(1.0f);
	result.col4.xy = p;
	result.col4.z = 0.0f;
	result.col4.w = 1.0f;
...
* @return The translation matrix.
*/
fpl_extern_inline Mat4f M4fTranslationV3(const Vec3f p) {
	Mat4f result = M4fInit(1.0f);
	result.col4.xyz = p;
	result.col4.w = 1.0f;
	return (result);
...
* @return The translation matrix.
*/
fpl_extern_inline Mat4f M4fTranslationV4(const Vec4f p) {
	Mat4f result = M4fInit(1.0f);
	result.col4 = p;
	return (result);
}
...
fpl_extern_inline Mat4f M4fTranslation(const Vec2f p) {
	return M4fTranslationV2(p);
}
//! C++ overload building a translation matrix from a Vec3f.
fpl_extern_inline Mat4f M4fTranslation(const Vec3f p) {
	return M4fTranslationV3(p);
}
//! C++ overload building a translation matrix from a Vec4f.
fpl_extern_inline Mat4f M4fTranslation(const Vec4f p) {
...
*/
fpl_extern_inline Mat4f M4fScaleScalar(const float s) {
	Mat4f result = M4fInit(1.0f);
	result.col1.x = s;
	result.col2.y = s;
	result.col3.z = s;
...
* @return The scale matrix.
*/
fpl_extern_inline Mat4f M4fScaleV2(const Vec2f s) {
	Mat4f result = M4fInit(1.0f);
	result.col1.x = s.x;
	result.col2.y = s.y;
	result.col3.z = 1.0f;
...
* @return The scale matrix.
*/
fpl_extern_inline Mat4f M4fScaleV3(const Vec3f s) {
	Mat4f result = M4fInit(1.0f);
	result.col1.x = s.x;
	result.col2.y = s.y;
	result.col3.z = s.z;
...
fpl_extern_inline Mat4f M4fScale(const float s) {
	return M4fScaleScalar(s);
}
//! C++ overload building a scale matrix from a Vec2f.
fpl_extern_inline Mat4f M4fScale(const Vec2f p) {
	return M4fScaleV2(p);
}
//! C++ overload building a scale matrix from a Vec3f.
fpl_extern_inline Mat4f M4fScale(const Vec3f p) {
	return M4fScaleV3(p);
}
#endif // __cplusplus
...
* @return The rotation matrix.
*/
fpl_extern_inline Mat4f M4fRotationX(const float angle) {
	float c = F32Cos(angle);
	float s = F32Sin(angle);
	Mat4f result;
...
* @return The rotation matrix.
*/
fpl_extern_inline Mat4f M4fRotationY(const float angle) {
	float c = F32Cos(angle);
	float s = F32Sin(angle);
	Mat4f result;
...
* @return The rotation matrix.
*/
fpl_extern_inline Mat4f M4fRotationZ(const float angle) {
	float c = F32Cos(angle);
	float s = F32Sin(angle);
	Mat4f result;
...
*/
fpl_extern_inline Mat4f M4fRotationZFromM2f(const Mat2f m) {
	Mat4f result;
...
* @return The matrix product a * b.
*/
fpl_extern_inline Mat4f M4fMult(const Mat4f a, const Mat4f b) {
	Mat4f result;
...
* @return The transposed matrix.
*/
fpl_extern_inline Mat4f M4fTranspose(const Mat4f m) {
	Mat4f result;
...
*/
fpl_extern_inline Mat4f M4fInverse(const Mat4f mat) {
...
fpl_extern_inline Mat4f operator *(const Mat4f &a, const Mat4f &b) {
...
* @return The transformed vector.
*/
fpl_extern_inline Vec4f V4fMultM4f(const Mat4f mat, const Vec4f v) {
...
* @return The dot product.
*/
fpl_extern_inline float QuatDot(const Quaternion a, const Quaternion b) {
...
*/
fpl_extern_inline Mat4f QuatToMat4(const Quaternion q) {
...
* @return The Euclidean length.
*/
fpl_extern_inline float QuatLength(const Quaternion q) {
...
*/
fpl_extern_inline Quaternion QuatNormalize(const Quaternion q) {
...
* @return The sum a + b.
*/
fpl_extern_inline Quaternion QuatAdd(const Quaternion a, const Quaternion b) {
...
* @return The difference a - b.
*/
fpl_extern_inline Quaternion QuatSub(const Quaternion a, const Quaternion b) {
	Quaternion result = QuatInit(a.w - b.w, a.x - b.x, a.y - b.y, a.z - b.z);
	return(result);
}
...
*/
fpl_extern_inline Quaternion QuatMultScalar(const Quaternion q, const float s) {
...
*/
fpl_extern_inline Vec3f QuatMultV3f(const Quaternion q, const Vec3f v) {
...
*/
fpl_extern_inline Vec4f QuatMultV4f(const Quaternion q, const Vec4f v) {
	Vec3f quatVector = V3fInit(q.x, q.y, q.z);
...
*/
fpl_extern_inline Quaternion QuatConjugate(const Quaternion quat) {
	float s = quat.s;
...
*/
fpl_extern_inline Quaternion QuatInverse(const Quaternion q) {
...
*/
fpl_extern_inline Vec3f QuatAxis(const Quaternion q) {
...
*/
fpl_extern_inline Quaternion QuatCross(const Quaternion a, const Quaternion b) {
...
*/
fpl_extern_inline Quaternion QuatLerp(const Quaternion a, const float t, const Quaternion b) {
...
*/
fpl_extern_inline float QuatAngle(const Quaternion q) {
...
*/
fpl_extern_inline Quaternion QuatFromAngleAxis(const float angle, const Vec3f axis) {
	Quaternion result;
	float const a = angle;
...
*/
fpl_extern_inline float QuatRoll(const Quaternion q) {
...
*/
fpl_extern_inline float QuatPitch(const Quaternion q) {
...
*/
fpl_extern_inline float QuatYaw(const Quaternion q) {
...
*/
fpl_extern_inline Quaternion QuatRotation(const Vec3f orig, const Vec3f dest) {
...
*/
fpl_extern_inline Pixel MakePixelFromRGBA(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a) {
...
* @return The initialized pixel.
*/
fpl_extern_inline Pixel MakePixelFromU32(const uint32_t rgba) {
	Pixel result = fplZeroInit;
...
*/
fpl_extern_inline uint32_t RGBA8FromRGBA(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a) {
...
* @return The packed RGBA value.
*/
fpl_extern_inline uint32_t RGBA8FromPixel(const Pixel pixel) {
...
*/
fpl_extern_inline uint32_t BGRA8FromRGBA(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a) {
...
* @return The packed BGRA value.
*/
fpl_extern_inline uint32_t BGRA8FromPixel(const Pixel pixel) {
...
* @return The packed BGRA value.
*/
fpl_extern_inline uint32_t BGRAPack4x8(const Vec4f unpacked) {
	uint32_t result = (
...
*/
fpl_extern_inline Vec4f BGRAUnpack4x8(const uint32_t packed) {
...
*/
fpl_extern_inline Pixel PixelPack(const Vec4f unpacked) {
	Pixel result = fplZeroInit;
...
* @return The color with components in the range [0, 1].
*/
fpl_extern_inline Vec4f PixelUnpack(const Pixel packed) {
	Vec4f result = fplZeroInit;
...
*/
fpl_extern_inline float SRGBToLinear(const float x) {
	if (x <= 0.0f)
		return 0.0f;
	else if (x >= 1.0f)
		return 1.0f;
...
*/
fpl_extern_inline float LinearToSRGB(const float x) {
	if (x <= 0.0f)
		return 0.0f;
	else if (x >= 1.0f)
		return 1.0f;
...
* @return The color with components in the range [0, 1].
*/
fpl_extern_inline Vec4f PixelToLinearRaw(const Pixel pixel) {
...
*/
fpl_extern_inline Vec4f PixelToLinearSRGB(const Pixel pixel) {
...
* @return The color with components in the range [0, 1].
*/
fpl_extern_inline Vec4f RGBAToLinearRaw(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a) {
...
* @return The color in linear space with components in the range [0, 1].
*/
fpl_extern_inline Vec4f RGBAToLinearSRGB(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a) {
	Pixel pixel = MakePixelFromRGBA(r, g, b, a);
...
*/
fpl_extern_inline Vec4f RGBAToLinearHex24(const uint32_t hexValue24) {
...
* @return The packed pixel.
*/
fpl_extern_inline Pixel LinearToPixelRaw(const Vec4f linear) {
	float r = linear.r;
	float g = linear.g;
	float b = linear.b;
	float a = linear.a;
...
*/
fpl_extern_inline Pixel LinearToPixelSRGB(const Vec4f linear) {
...
*/
fpl_extern_inline Vec4f V4fDivideScalar(const Vec4f vec, const float divisor) {
...
*/
fpl_extern_inline Vec2f V2fUnproject(const Vec2i screenPos, const Mat4f mvp, const Viewport4i viewport) {
...
*/
fpl_extern_inline Vec2i V2iProject(const Vec2f worldPos, const Mat4f mvp, const Viewport4i viewport) {
...
*/
fpl_extern_inline Viewport4i VP4iComputeByAspect(const Vec2i screenSize, const float targetAspect) {
...
* @return The centered viewport matching the target aspect ratio.
*/
fpl_extern_inline Viewport4f VP4fComputeByAspect(const Vec2f screenSize, const float targetAspect) {
...
* @return The diagonal matrix.
*/
fpl_extern_inline Mat3f M3fInit(const float value) {
	Mat3f result;
...
* @return The identity matrix.
*/
fpl_extern_inline Mat3f M3fIdentity() {
	return M3fInit(1.0f);
}
...
* @return A copy of the input matrix.
*/
fpl_extern_inline Mat3f M3fCopy(const Mat3f other) {
...
fpl_extern_inline Mat3f M3f() {
	return M3fIdentity();
}
//! C++ overload constructing a diagonal Mat3f.
fpl_extern_inline Mat3f M3f(const float value) {
	return M3fInit(value);
}
//! C++ overload constructing a Mat3f from another Mat3f.
fpl_extern_inline Mat3f M3f(const Mat3f &other) {
...
*/
fpl_extern_inline Mat3f M3fFromMat4(const Mat4f m) {
	Mat3f result;
...
* @return The transformed vector.
*/
fpl_extern_inline Vec3f V3fMultM3f(const Mat3f m, const Vec3f v) {
	Vec3f result;
...
* @return The matrix product a * b.
*/
fpl_extern_inline Mat3f M3fMult(const Mat3f a, const Mat3f b) {
	Mat3f result;
...
* @return The transposed matrix.
*/
fpl_extern_inline Mat3f M3fTranspose(const Mat3f m) {
	Mat3f result;
...
* @return The translation matrix.
*/
fpl_extern_inline Mat3f M3fTranslationV2(const Vec2f p) {
...
* @return The scale matrix.
*/
fpl_extern_inline Mat3f M3fScaleV2(const Vec2f s) {
	Mat3f result = M3fIdentity();
	result.r[0][0] = s.x;
	result.r[1][1] = s.y;
	return(result);
...
* @return The rotation matrix.
*/
fpl_extern_inline Mat3f M3fRotationZ(const float angle) {
	float c = F32Cos(angle);
	float s = F32Sin(angle);
	Mat3f result;
