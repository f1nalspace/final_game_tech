...
static const char *ClipboardGetFunc(void *user) {
	char *result = fpl::window::GetClipboardAnsiText(clipboardBuffer, FPL_ARRAYCOUNT(clipboardBuffer));
	return(result);
}
static void ClipboardSetFunc(void *user, const char *text) {
	fpl::window::SetClipboardText(text);
}
...
static void ImGUIKeyEvent(uint64_t keyCode, fpl::window::Key mappedKey, fpl::window::KeyboardModifierFlags modifiers, bool down) {
	ImGuiIO& io = ImGui::GetIO();
	if (mappedKey != fpl::window::Key::Key_None) {
		io.KeysDown[(uint32_t)mappedKey] = down;
	} else {
		io.KeysDown[keyCode] = down;
	}
	io.KeyCtrl = modifiers & fpl::window::KeyboardModifierFlags::Ctrl;
	io.KeyShift = modifiers & fpl::window::KeyboardModifierFlags::Shift;
	io.KeyAlt = modifiers & fpl::window::KeyboardModifierFlags::Alt;
	io.KeySuper = modifiers & fpl::window::KeyboardModifierFlags::Super;
}
...
int main(int argc, char **args) {
	...
	int result = 0;
	fpl::Settings settings = fpl::DefaultSettings();
	fpl::strings::CopyAnsiString("ImGUI Example", settings.window.windowTitle, FPL_ARRAYCOUNT(settings.window.windowTitle) - 1);
	settings.window.windowWidth = 1280;
	settings.window.windowHeight = 720;
	settings.video.driver = fpl::VideoDriverType::OpenGL;
	if (fpl::InitPlatform(fpl::InitFlags::Video, settings)) {
		...
		double lastTime = fpl::timings::GetHighResolutionTimeInSeconds();
		...
		while (IsWindowRunning()) {
			...
			double currentTime = fpl::timings::GetHighResolutionTimeInSeconds();
...
int main(int argc, char *args[]) {
    if (fpl::InitPlatform(fpl::InitFlags::Video)) {
        ...
        fpl::console::ConsoleFormatOut("%s\n", renderer);
        fpl::ReleasePlatform();
        return 0;
    } else {
        return -1;
