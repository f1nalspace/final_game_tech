...
float GetShortestAngleDist(float a0, float a1) {
	float max = Pi32 * 2;
	...
	if (state->waveState == WaveState::Running) {
		...
		for (size_t towerIndex = 0; towerIndex < state->towerCount; ++towerIndex) {
			...
			if (tower.hasTarget) {
...
#else
				Vec2f distanceToEnemy = Vec2Normalize(enemy->position - tower.position);
#endif
				...
				float angleDistance = GetShortestAngleDist(tower.facingAngle, angleToEnemy);
				...
				if (!tower.gunIsRotating) {
					...
					tower.targetAngle[1] = tower.facingAngle + angleDistance;
			...
			// Fire only when we have a target in sight
			{
				...
				for (size_t enemyIndex = 0; enemyIndex < state->enemyCount; ++enemyIndex) {
					...
					Creep &enemy = state->enemies[enemyIndex];
					Vec2f distance = (enemy.position + enemy.facingDirection * enemy.speed) - tower.position;
					float maxDistance = Vec2Length(distance) + enemy.data->collisionRadius * 10.0f;
					LineCastInput input = {};
					...
					bool willHit = LineCastCircle(input, enemy.position, enemy.data->collisionRadius, output);
					if (willHit) {
