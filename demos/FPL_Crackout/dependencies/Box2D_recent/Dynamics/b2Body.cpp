...
void b2Body::SetType(b2BodyType type)
{
	...
	{
		b2_int32 proxyCount = f->m_proxyCount;
		for (b2_int32 i = 0; i < proxyCount; ++i)
		{
...
b2Fixture* b2Body::CreateFixture(const b2Shape* shape, b2_f32 density)
{
	b2FixtureDef def;
	def.shape = shape;
	def.density = density;
...
void b2Body::SetTransform(const b2Vec2& position, b2_f32 angle)
{
	b2Assert(m_world->IsLocked() == false);
	if (m_world->IsLocked() == true)
	{
...
void b2Body::Dump()
{
	b2_int32 bodyIndex = m_islandIndex;
