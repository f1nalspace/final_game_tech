	...
	for (;;)
	{
		...
		for (b2Contact* c = m_contactManager.m_contactList; c; c = c->m_next)
		{
			...
			else
			{
				...
				b2_f32 alpha0 = bA->m_sweep.alpha0;
...
void b2World::Step(b2_f32 dt, b2_int32 velocityIterations, b2_int32 positionIterations)
{
	b2Timer stepTimer;
...
{
	b2_f32 RayCastCallback(const b2RayCastInput& input, b2_int32 proxyId)
	{
		void* userData = broadPhase->GetUserData(proxyId);
		b2FixtureProxy* proxy = (b2FixtureProxy*)userData;
		b2Fixture* fixture = proxy->fixture;
		b2_int32 index = proxy->childIndex;
		b2RayCastOutput output;
		bool hit = fixture->RayCast(&output, input, index);

		if (hit)
		{
			b2_f32 fraction = output.fraction;
	...
	case b2Shape::e_polygon:
		{
			...
			b2_int32 vertexCount = poly->m_count;
			b2Assert(vertexCount <= b2_maxPolygonVertices);
			b2Vec2 vertices[b2_maxPolygonVertices];

			for (b2_int32 i = 0; i < vertexCount; ++i)
			{
...
void b2World::DrawDebugData()
{
	...
	b2_uint32 flags = g_debugDraw->GetFlags();
	...
	if (flags & b2Draw::e_aabbBit)
	{
		...
		for (b2Body* b = m_bodyList; b; b = b->GetNext())
		{
			...
			for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
			{
				for (b2_int32 i = 0; i < f->m_proxyCount; ++i)
				{
...
b2_f32 b2World::GetTreeQuality() const
{
