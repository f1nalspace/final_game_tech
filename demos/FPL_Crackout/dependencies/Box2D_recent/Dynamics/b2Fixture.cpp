...
void b2Fixture::Create(b2BlockAllocator* allocator, b2Body* body, const b2FixtureDef* def)
{
	...
	// Reserve proxy space
	b2_int32 childCount = m_shape->GetChildCount();
	m_proxies = (b2FixtureProxy*)allocator->Allocate(childCount * sizeof(b2FixtureProxy));
	for (b2_int32 i = 0; i < childCount; ++i)
	{
...
void b2Fixture::Dump(b2_int32 bodyIndex)
{
	...
	b2Log("    fd.filter.categoryBits = b2_uint16(%d);\n", m_filter.categoryBits);
	b2Log("    fd.filter.maskBits = b2_uint16(%d);\n", m_filter.maskBits);
	b2Log("    fd.filter.groupIndex = b2_int16(%d);\n", m_filter.groupIndex);
	...
	case b2Shape::e_polygon:
		{
			...
			for (b2_int32 i = 0; i < s->m_count; ++i)
			{
