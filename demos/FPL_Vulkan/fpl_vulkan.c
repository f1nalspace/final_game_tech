...
static void VulkanTemporaryRecordBuffer(const VulkanLogicalDevice *logicalDevice, VulkanFrame *frame) {
    ...
    for (uint32_t imageIndex = 0; imageIndex < frame->swapChain.imageCount; ++imageIndex) {
        ...
        deviceApi->vkCmdClearColorImage(cmdBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearColor, 1,
...
// Helper function to create graphics pipeline with shaders
static bool VulkanCreateGraphicsPipeline(
    VkAllocationCallbacks *allocator,
    VulkanLogicalDevice *logicalDevice,
    VkShaderModule vertShaderModule,
    VkShaderModule fragShaderModule,
    ...
    // Pipeline layout
    VkPipelineLayoutCreateInfo pipelineLayoutInfo = fplZeroInit;
    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;

    if (deviceApi->vkCreatePipelineLayout(logicalDevice->logicalDeviceHandle, &pipelineLayoutInfo, allocator, outLayout)
        != VK_SUCCESS) {
        return (false);
    }

    // Fixed function stages
    VkPipelineShaderStageCreateInfo vertShaderStageInfo = fplZeroInit;
    vertShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    vertShaderStageInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
    vertShaderStageInfo.module = vertShaderModule;
    vertShaderStageInfo.pName = "main";

    VkPipelineShaderStageCreateInfo fragShaderStageInfo = fplZeroInit;
    ...
    // Vertex input (just a triangle strip)
    ...
    VkPipelineVertexInputStateCreateInfo vertexInputInfo = fplZeroInit;
    ...
    VkViewport viewport = fplStructInit(VkViewport, 0.0f, 0.0f, (float)extent.width, (float)extent.height, 0.0f, 1.0f);
    VkRect2D scissor = fplStructInit(VkRect2D, {0, 0}, extent);
    ...
    VkPipeline graphicsPipeline;
    if (deviceApi->vkCreateGraphicsPipelines(logicalDevice->logicalDeviceHandle, VK_NULL_HANDLE, 1, &pipelineInfo,
                                             allocator, &graphicsPipeline) != VK_SUCCESS) {
        deviceApi->vkDestroyPipelineLayout(logicalDevice->logicalDeviceHandle, *outLayout, allocator);
        return (false);
    }
...
// Helper function to destroy graphics pipeline
static void VulkanDestroyGraphicsPipeline(
    VkAllocationCallbacks *allocator,
    VulkanLogicalDevice *logicalDevice,
    VkPipeline *pipeline,
    ...
    if (*pipeline != VK_NULL_HANDLE) {
        deviceApi->vkDestroyPipeline(logicalDevice->logicalDeviceHandle, *pipeline, allocator);
        *pipeline = VK_NULL_HANDLE;
    }
    if (*layout != VK_NULL_HANDLE) {
        deviceApi->vkDestroyPipelineLayout(logicalDevice->logicalDeviceHandle, *layout, allocator);
