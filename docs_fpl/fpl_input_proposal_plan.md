...
4. Allow user-controlled enable/disable of individual backends through `fplInputSettings`.
5. Define a small, uniform backend interface with: load, init, release, device enumeration, per-device polling, event handling.
6. Let the window subsystem **forward** native events to the input subsystem (for device-hotplug notification, keyboard/mouse polling shortcuts, etc.) instead of implementing input itself.
7. Stay simple: fewer layers than the audio backend, no async main loop, no threading requirement.
8. Keep the public API (`fplPollKeyboardState`, `fplPollMouseState`, `fplPollGamepadStates`) source-compatible.
...
+-----------------------------------------------------------+
    |              |              |              |
    v              v              v              v
 XInput         DirectInput    Win32Raw      X11/Evdev/...
